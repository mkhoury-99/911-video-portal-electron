...
export function TouchTarget({ children, className, ...props }) {
  return (
    <span
      {...props}
      className={cn(
        className,
        "absolute left-1/2 top-1/2 size-[max(100%,2.75rem)] -translate-x-1/2 -translate-y-1/2 [@media(pointer:coarse)]:size-[max(100%,2.75rem)]"
      )}
    >
      {children}
    </span>
